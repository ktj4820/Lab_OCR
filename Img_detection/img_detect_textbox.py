...
def detect_textbox(raw_img_input):
    ...
    def find_real_start_row(real_image_height):
        ...
        while find_real_start_row_mark < real_image_height - 2:
            ...
            if (row_white_num[real_image_height - find_real_start_row_mark] if arg else row_black_num[real_image_height - find_real_start_row_mark]) >= \
                    (0.85 * row_white_num_max if arg else 0.85 * row_black_num_max) and \
                    (row_white_num[real_image_height - find_real_start_row_mark - 1] <= 0.3 * row_white_num_max):
    ...
    def find_real_height(initial_real_image_height):
        ...
        while find_real_end_row_mark < initial_image_height - 2:
            ...
            if (row_white_num[initial_image_height-find_real_end_row_mark] if arg else row_black_num[find_real_end_row_mark]) > \
                    ...
                    row_white_num[initial_image_height - find_real_end_row_mark - 1] <= 0.2 * row_white_num_max:
    ...
    def find_real_end_column(initial_real_image_width):
        ...
        while find_real_end_row_mark < initial_real_image_width - 2:
            ...
            即将该行看做出现字符的起始行。
            对于白底黑字情况，则反之'''
            if (column_white_num[initial_real_image_width - find_real_end_row_mark]) >= 0.7 * column_white_num_max and \
                    column_white_num[initial_real_image_width - find_real_end_row_mark - 1] <= \
                    0.3 * column_white_num_max:
    ...
    # 寻找文本框起始行与结束行
    real_image_width = find_real_height(image_height)
    img_textbox_start = find_real_start_row(image_height)

    while img_textbox_start < image_height - 2:
        img_textbox_start += 1
        row_find_end = real_image_width
