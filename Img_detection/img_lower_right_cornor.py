...
def char_split(raw_img_input, img_compression_height):
    """
    分割图像，完成字符级切割
    :param raw_img_input:
    :param img_compression_height:
    :return:
    """
    # 分割图像，提取字符行
    ...
    raw_img_input = img_compression(raw_img_input, img_compression_height)
...
def detect_row_img_color(image):
    ...
    # bgr_light_blue = [176, 181, 27]  # 淡蓝判定基准
    bgr_light_blue = [162, 178, 10]
    ...
    def check_color(raw_img_input,decision_threshold=0.01):
        ...
        if (total_white_num if arg else total_black_num) > (total_black_num * decision_threshold if arg else total_white_num):
    ...
    if check_color(detect_img_submodule(img, bgr_red, thresh_red=60)):
        ...
        cv2.imshow("the raw light blue img input", img)
        cv2.waitKey(0)

        blue_img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
        # bright_hsv = cv2.cvtColor(bright, cv2.COLOR_BGR2HSV)
        # convert 1D array to 3D, then convert it to LAB and take the first element
        # python
        bgr_light_blue = [162, 178, 10]    # 淡蓝色判定基准
        bgr = bgr_light_blue
        thresh = 60
        ...
        mask_hsv = cv2.inRange(blue_img_hsv, min_hsv, max_hsv)
        result_hsv = cv2.bitwise_and(blue_img_hsv, blue_img_hsv, mask=mask_hsv)

        # 依照黄色字体对应亮度进行二值化处理
        final_binary_img = gen_thresh_img(result_hsv, threshold=100, mode='dynamic')
...
if __name__ == '__main__':
    ...
    # input_image = "../Img_processed/106/img_three_one.jpg"
    # raw_img = cv2.imread(input_image)
    # char_split(raw_img)
    input_image_mark = 97
    input_image_list = ["../Img_processed/" + str(input_image_mark) + "/img_three_four_one.jpg",
                        "../Img_processed/" + str(input_image_mark) + "/img_three_four_two.jpg"]

    for input_image in input_image_list:
        raw_img = cv2.imread(input_image)
        char_split(raw_img, img_compression_height=77)
