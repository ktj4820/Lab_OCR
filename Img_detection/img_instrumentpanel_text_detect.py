...
def char_split(raw_img_input):
    ...
    # 依据列，寻找起始点
    def find_start_point(start_column, char_width):
    ...
    char_image_height = thresh_img.shape[0]
    char_width = thresh_img.shape[1]
    ...
    while column_mark < char_width - 2:
        ...
        #     continue
        # 判断起始点
        if (column_white_num[column_mark] if char_arg else column_black_num[column_mark]) > (1.5 if char_arg else 1.5):

            column_find_end = find_start_point(column_mark)
