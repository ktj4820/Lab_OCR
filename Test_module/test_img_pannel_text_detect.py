...
def identify_panel_text(panel_text_input):
    ...
    if avg_column_white < 2.7:
        print("This's no text in panel")
    elif avg_column_white < 4.75:
...
def detect_textbox(raw_img_input):
        ...
        while find_real_start_row_mark < real_image_height - 2:
            ...
            if (row_white_num[real_image_height - find_real_start_row_mark] if arg else
                row_black_num[real_image_height - find_real_start_row_mark]) >= \
                    ...
                    (row_white_num[real_image_height - find_real_start_row_mark - 1] < 0.65 * row_white_num_max):
    ...
    def find_real_start_column(initial_real_image_width):
        ...
        while find_real_start_column_mark < initial_real_image_width - 2:
            ...
            if (column_white_num[find_real_start_column_mark] if arg else row_black_num[find_real_start_column_mark]) \
                    >= (0.7 * column_white_num_max if arg else 0.5 * column_black_num_max) and \
                    (column_white_num[find_real_start_column_mark + 1] <= 0.7 * column_white_num_max):
    ...
    raw_img_input = img_compression(raw_img_input, 115)
    print('img_height: {}, img_width: {}'.format(raw_img_input.shape[0], raw_img_input.shape[1]))
    ...
    while img_textbox_start < image_height - 2:
        ...
        # raw_image_split_row
        textbox = raw_image_split_row[0:char_image_height, real_start_column:real_end_column]
...
def detect_row_img_color(image):
    ...
    def check_color(raw_img_input, decision_threshold=0.07):
        """
        依据图片内白色（字体）像素点占总像素点的比率，判定所截图片是否对应所判定颜色
        :param raw_img_input:
        :param decision_threshold:
...
if __name__ == '__main__':
    ...
    input_image_list = ["../Img_processed/" + str(input_image_mark) + "/img_top_left_panel.jpg",
                        ...
                        "../Img_processed/" + str(input_image_mark) + "/img_lower_right_panel.jpg"]
