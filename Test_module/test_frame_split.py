    ...
    def find_end_row(start_row):
        ...
        for m in range(start_row + 1, image_height + 1):
            if (row_black_num[m] if arg else row_white_num[m]) > \
                    (0.91 * row_black_num_max if arg else 0.91 * row_white_num_max):  # 0.95这个参数请多调整，对应下面的0.05
                end_row = m
                break
        return end_row
    ...
    while row_mark < image_height - 2:
        ...
        if (row_white_num[row_mark] if arg else row_black_num[row_mark]) > \
            ...
            if row_find_end - row_mark > 5:  # 要求切割字符行需要有5行以上的距离是为了保证排除直线的干扰
                ...
                row_split_start = row_mark if row_mark > 0 else row_mark
                row_split_end = row_find_end if row_find_end < image_height else row_find_end
                ...
                # cv2.imwrite('../Img_processed/row_split_{}.jpg'.format( -1), image_split_row)
                ...
                def find_end_column(start_column):
                    # 这里要尤其注意，因为 start_column 输入时候减去了2，如果这里直接+1，
                    # 会造成下一列立马结束（因为出现背景列），导致 end_column < start_column
